module.exports = function(open, read, close) {
  ...
  function fill(callback) {
    if (channel === CHANNEL_AFTER_END) throw new Error("cannot fill after ended");
    if (channel === CHANNEL_BEFORE_OPEN) return fillCallback = callback, void reader_open();
  ...
  function afterFill(newError, readLength) {
    ...
    if (error = newError) return void end(oldFillCallback.bind(null, error));

    // If no more bytes were available, then we’ve reached the end, so close.
    if (!readLength) return void end(oldFillCallback);

    // Otherwise mark the read bytes as available.
    bufferAvailable += readLength;
    oldFillCallback(null, readLength);
  }
  ...
  function reader_end(callback) {
    ...
    if (channel === CHANNEL_AFTER_END) throw new Error("cannot end after already ended");
    end(callback || rethrow);
  }

  function end(callback) {
    var oldChannel = channel;
    channel = CHANNEL_AFTER_END;
