module.exports = function(open, write, close) {
  ...
  function drain(callback) {
    if (channel === CHANNEL_AFTER_END) throw new Error("cannot drain after ended");
    if (channel === CHANNEL_BEFORE_OPEN) return drainCallback = callback, void writer_open();
  ...
  function writer_write(data) {
    ...
    if (channel === CHANNEL_AFTER_END) throw new Error("cannot write after ended");
  ...
  function writer_end(callback) {
    ...
    drain(function(error) {
      ...
      close(oldChannel, callback || rethrow);
    });
  }
